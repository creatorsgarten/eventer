...
export class AgendaEntityProps {
 ...
 get duration(): number {
    return (
      new Date(this.agenda.end).getTime() -
      new Date(this.agenda.start).getTime()
    ) / 60000; // in minutes
}
 static validateSequential(agenda: AgendaType[]) {
    ...
    for (let i = 1; i < agenda.length; i++) {
      ...
      if (prevEnd !== currentStart) {
        throw new Error(`Agenda at index ${i} does not start immediately after the previous agenda. Expected start: ${new Date(prevEnd).toISOString()}, got: ${new Date(currentStart).toISOString()}`);
      }
    }
  }
}
