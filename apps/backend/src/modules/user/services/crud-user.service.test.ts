...
import { users, type UserType } from "../user.model";
import {
  createUser,
  listUsers,
  getUserById,
...
describe("User Service", () => {
  ...
  describe("createUser", () => {
    ...
    it("should throw error when creating user with existing email", async () => {
      ...
      await expect(
        createUser(userRepository, duplicateEmailUser)
    ...
    it("should throw error when creating user with existing username", async () => {
      ...
      await expect(
        createUser(userRepository, duplicateUsernameUser)
  ...
  describe("updateUser", () => {
    ...
    it("should throw error when updating to existing email", async () => {
      ...
      await expect(
        updateUser(userRepository, user1.id, updateData)
    ...
    it("should throw error when updating to existing username", async () => {
      ...
      await expect(
        updateUser(userRepository, user1.id, updateData)
      ).rejects.toThrow("Another user with this username already exists");
    });
    ...
    it("should throw error when updating non-existent user", async () => {
      ...
      await expect(
        updateUser(userRepository, "non-existent-id", updateData)
  ...
  describe("deleteUser", () => {
    ...
    it("should throw error when deleting non-existent user", async () => {
      await expect(
        deleteUser(userRepository, "non-existent-id")
      ).rejects.toThrow("User not found");
    });
  });
});
